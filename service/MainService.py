...
class MainService:
    ...
    def compare_images(self, img_list1, img_list2):
        ...
        gen = (img1 for img1 in img_list1 if similar_images < minim_similar_images)
    ...
    def compare_descriptions(self, description_info1, description_info2):
        ...
        if same_info / len(description_info1.keys()) >= 0.75:
            return True
        else:
            return False

    def is_same_announcement(self, img_list1, img_list2, description1, description2):
        print("First image list: " + ', '.join(str(elem) for elem in img_list1))
        print("Second image list: " + ', '.join(str(elem) for elem in img_list2))
